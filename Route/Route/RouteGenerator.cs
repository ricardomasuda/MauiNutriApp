...
public class RouteGenerator : IIncrementalGenerator {
    ...
    private void Execute(SourceProductionContext context,
        ...
        (Compilation? compilation, ImmutableArray<ClassDeclarationSyntax> classes) = compilationTuple;

        INamedTypeSymbol? attributeAutoGenSymbol = compilation.GetTypeByMetadataName(Constants.AutoRoutesFullName);
            ...
            .Select(t => new {
                ...
                AttributeData = t
                    ...
                    .FirstOrDefault(ad =>
                        ...
                        ad.AttributeClass.Equals(attributeAutoGenSymbol, SymbolEqualityComparer.Default))
            })
        ...
        if (classWithAutoGenAttributeData.AttributeData?.ConstructorArguments.FirstOrDefault()
                .Value is not string suffix || string.IsNullOrWhiteSpace(suffix)) {
        ...
        string namespaceName = classWithAutoGenAttributeData.Class.ContainingNamespace.ToDisplayString();

        List<ClassDeclarationSyntax> routeClassDeclarationSyntaxList =
            classes.Where(c => c.Identifier.Text.EndsWith(suffix)).ToList();
        List<string> routeNameList =
            routeClassDeclarationSyntaxList.Select(pageClass => pageClass.Identifier.Text).ToList();

        AddExtraRoutes(context, compilation, routeNameList);

        string source = BuildSource(routeNameList, namespaceName);

        context.AddSource(Constants.RoutesGenFileName, source);
    }

    private void AddExtraRoutes(SourceProductionContext context, Compilation compilation,
        ICollection<string> routeNameList) {
        INamedTypeSymbol? attributeExtraRouteSymbol = compilation.GetTypeByMetadataName(Constants.ExtraRouteFullName);
        ...
        foreach (AttributeData? attributeData in classesWithExtraRouteAttributeData.SelectMany(
                     classWithExtraRouteAttributeData =>
                         classWithExtraRouteAttributeData.AttributeData)) {
    ...
    private static string BuildSource(IReadOnlyCollection<string> routeNameList, string namespaceName) {
        IEnumerable<string> routeClassDeclarationStrings =
            routeNameList.Select(page => $"public const string {page} = \"{page}\";");
        string routeClassDeclarationsString = string.Join("\n        ", routeClassDeclarationStrings);
    ...
    private static IEnumerable<INamedTypeSymbol> GetAllClasses(INamespaceSymbol namespaceSymbol) {
        foreach (INamespaceOrTypeSymbol? member in namespaceSymbol.GetMembers()) {
            if (member is INamespaceSymbol childNamespace) {
                foreach (INamedTypeSymbol? childClass in GetAllClasses(childNamespace)) {
